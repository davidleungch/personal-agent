...
function sameEvent(event: CalendarEvent, expected: Omit<CalendarEvent, "id">): boolean {
  return event.summary === expected.summary && event.start === expected.start && event.end === expected.end && event.description === expected.description && event.location === expected.location;
}
...
export function createCalendarToolDefinitions(transport: CalendarTransport): readonly AnyToolDefinition[] {
  ...
  const update = defineTool({
    execute: async (input: z.infer<typeof updateInput>, context: ToolExecutionContext) => { const expected = desired(input); const current = await transport.get(input.calendarId, input.eventId, context.signal); if (current && sameEvent(current, expected)) return { data: output(current), externalId: current.id, retryable: false, status: "success" as const }; context.reportSideEffectStarted(); await transport.update(input.calendarId, input.eventId, expected, context.signal); const verified = await transport.get(input.calendarId, input.eventId, context.signal); if (!verified || !sameEvent(verified, expected)) return { externalId: input.eventId, failureClass: "verification_failed" as const, retryable: false, status: "unknown" as const }; return { data: output(verified), evidence: [{ payload: { externalId: verified.id, method: "read_back" }, type: "calendar_event" }], externalId: verified.id, retryable: false, status: "success" as const }; },
    idempotencyKey: (input: z.infer<typeof updateInput>) => stableKey("calendar.update", [input.calendarId, input.eventId, input.summary, input.start, input.end]), inputSchema: updateInput, integration: "google" as const, name: "calendar.update_event", outputSchema: eventOutput, permission: "external_write" as const, retryPolicy,
    ...
    verify: async (input: z.infer<typeof updateInput>, context: ToolExecutionContext) => { const event = await transport.get(input.calendarId, input.eventId, context.signal); if (!event) return { status: "absent" as const }; return sameEvent(event, desired(input)) ? { data: output(event), externalId: event.id, status: "exists" as const } : { status: "absent" as const }; }
  });
